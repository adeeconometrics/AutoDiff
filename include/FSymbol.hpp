...
public:
  FSymbol(const T &_val, const T &_dot) : m_val{_val}, m_dot{_dot} {}

  FSymbol(const T &_dot) : m_dot{_dot} {}
...
auto operator/(const FSymbol<T> &lhs, const FSymbol<T> &rhs) -> FSymbol<T> {
  const auto df =
      (lhs.dot() * rhs.value() - lhs.value() * rhs.dot()) / pow(rhs.value(), 2);
...
auto pow(const FSymbol<T> &base, const FSymbol<T> &exp) -> FSymbol<T> {
  const auto df_base =
      log(base.value()) * pow(base.value(), exp.value()) * exp.dot();
  return {pow(base.value(), exp.value()), df_base};
}

template <typename T, typename U>
requires(AbstractRealVector<T> || std::is_floating_point_v<T>) &&
    std::is_arithmetic<U>::value auto pow(const FSymbol<T> &base, U exp)
        -> FSymbol<T> {
...
auto asin(const FSymbol<T> &rhs) noexcept -> FSymbol<T> {
  const auto value = asin(rhs.value());
  const auto df = 1.0 / sqrt(1.0 - pow(rhs.value(), 2)) * rhs.dot();
  return {value, df};
}
...
auto acos(const FSymbol<T> &rhs) noexcept -> FSymbol<T> {
  const auto value = acos(rhs.value());
  const auto df = -1.0 / sqrt(1 - pow(rhs.value(), 2)) * rhs.dot();
  return {value, df};
}
...
auto atan(const FSymbol<T> &rhs) noexcept -> FSymbol<T> {
  const auto value = atan(rhs.value());
  const auto df = 1.0 / (pow(rhs.value(), 2) + 1) * rhs.dot();
  return {value, df};
}
...
auto acot(const FSymbol<T> &rhs) noexcept -> FSymbol<T> {
  const auto value = 1.0 / atan(rhs.value());
  const auto df = -1.0 / (pow(rhs.value(), 2) + 1) * rhs.dot();
  return {value, df};
}
...
auto asec(const FSymbol<T> &rhs) noexcept -> FSymbol<T> {
  const auto value = 1.0 / acos(rhs.value());
  const auto df =
      1.0 / (abs(rhs.value()) * sqrt(pow(rhs.value(), 2) - 1)) * rhs.dot();
  return {value, df};
}
...
auto acsc(const FSymbol<T> &rhs) noexcept -> FSymbol<T> {
  const auto value = 1.0 / asin(rhs.value());
  const auto df =
      -1.0 / (abs(rhs.value()) * sqrt(pow(rhs.value(), 2) - 1.0)) * rhs.dot();
  return {value, df};
}
...
auto asinh(const FSymbol<T> &rhs) noexcept -> FSymbol<T> {
  const auto value = asinh(rhs.value());
  const auto df = 1.0 / sqrt(1.0 + pow(rhs.value(), 2)) * rhs.dot();
  return {value, df};
}
...
auto acosh(const FSymbol<T> &rhs) noexcept -> FSymbol<T> {
  const auto value = acosh(rhs.value());
  const auto df = 1.0 / sqrt(pow(rhs.value(), 2) - 1) * rhs.dot();
  return {value, df};
}
...
auto atanh(const FSymbol<T> &rhs) noexcept -> FSymbol<T> {
  const auto value = atanh(rhs.value());
  const auto df = 1.0 / (1.0 - pow(rhs.value(), 2)) * rhs.dot();
  return {value, df};
}
...
auto acoth(const FSymbol<T> &rhs) noexcept -> FSymbol<T> {
  const auto value = 1.0 / atanh(rhs.value());
  const auto df = 1.0 / (1.0 - pow(rhs.value(), 2)) * rhs.dot();
  return {value, df};
}
...
auto asech(const FSymbol<T> &rhs) noexcept -> FSymbol<T> {
  const auto value = 1.0 / acosh(rhs.value());
  const auto df =
      -1.0 / (rhs.value() * sqrt(1.0 - pow(rhs.value(), 2))) * rhs.dot();
  return {value, df};
}
...
auto acsch(const FSymbol<T> &rhs) noexcept -> FSymbol<T> {
  const auto value = 1.0 / asinh(rhs.value());
  const auto df =
      -1.0 / (abs(rhs.value()) * sqrt(1 + pow(rhs.value(), 2))) * rhs.dot();
  return {value, df};
}
